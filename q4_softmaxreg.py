...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentVector = [wordVectors[tokens[word]] for word in sentence]
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    labels_onehot = np.zeros(prob.shape)
    # labels_onehot[range(N), labels] = 1.0
    labels_onehot[np.arange(len(labels)), labels] = 1.0
    grad = (np.dot(features.T, prob - labels_onehot)/ N) + (regularization * weights)
